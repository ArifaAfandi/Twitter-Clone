      ...
      <div className="sticky top-16 text-gray-700 space-y-3 bg-gray-100 rounded-xl pt-2 w-[90%] xl:w-[75%]">
        ...
        <AnimatePresence>
          {randomUsersResults.slice(0, randomUserNum).map((randomUser) => (
            <motion.div key={randomUser.login.uuid} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1 }}>
              <div className="flex items-center px-4 py-2 cursor-pointer hover:bg-gray-200 transition duration-500 ease-out" key={randomUser.login.uuid}>
                <img className="rounded-full" width="40" src={randomUser.picture.thumbnail} alt="user" />
                <div className="truncate text-overflow:ellipsis ml-4 leading-5">
                  <h4 className="font-bold hover:underline text-[14px]">{randomUser.login.username}</h4>
                  <h5 className="text-[13px] text-gray-500 truncate text-overflow:ellipsis">{randomUser.name.first + " " + randomUser.name.last}</h5>
                </div>
